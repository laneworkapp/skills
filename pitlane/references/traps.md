...
| end a record with the question for the human | the app draws option buttons only on a real ask, so a handle buried after a record renders its options as plain bullets with nothing to click (Lab board, 2026-09-26, a shaper's 277-word record closing on `@rzen What per-source limits…`). Split it: the record, then the ask a second later, `scripts/lint-ask.sh` clean |
